...
import { createContainer, seedReferenceData, type AuthContext, type Container } from "@ewayvo/core";
import { einvoices, ewayBills, invoices, users } from "@ewayvo/database";
import { computeValidity } from "@ewayvo/shared";

const DEMO_EMAIL = "owner@demo.ewayvo.in";
const DEMO_PASSWORD = "EwayvoDemo2026!";

const daysAgo = (days: number): Date => new Date(Date.now() - days * 86_400_000);

async function main(): Promise<void> {
  const container = createContainer({ processName: "ewayvo-seed" });
  const { database, logger, masters, invoices: invoiceService } = container;

  /*
   * Seeding legitimately spans tenants: it creates the demo business and then
   * writes as that business. It therefore runs in an explicit system scope,
   * the same mechanism migrations and the job claimer use. Without one every
   * query fails closed, which is the correct behaviour for application code
   * but wrong for a provisioning script.
   */
  try {
    await database.withSystemScope("seed", async () => {
      ...
      const plant =
        branchList.find((b) => b.kind === "plant") ??
        (await masters.createBranch(ctx, {
          gstinId: gstin.id,
          code: "PLT-01",
      ...
      const hosurSite =
        meridianDetail.addresses[0] ??
        (await masters.addPartyAddress(ctx, meridian!.id, {
      ...
      const catalogue = [
        ...
        {
          name: "Fabrication and erection charges",
          sku: "SVC-FAB",
          hsn: "998399",
          unit: "NOS",
          price: 25000,
          rate: 18,
          description: "On-site fabrication and erection",
          service: true,
        },
      ];
      ...
      for (const item of catalogue) {
        ...
        const product =
          found ??
          (await masters.createProduct(ctx, {
            name: item.name,
      ...
      const transporter =
        transporterList.items[0] ??
        (await masters.createTransporter(ctx, {
      ...
      const complex = await invoiceService.createDraft(
        ctx,
        base({
          ...
          placeOfSupply: "29",
          lines: [
            line("MS Structural Beam ISMB 200", 12.5, 2),
            line("HT Bolt M20 x 80 (Grade 8.8)", 1500),
            line("Red Oxide Primer 20L", 8),
          ],
          charges: [
            {
              label: "Freight to Hosur",
              kind: "freight",
              hsnSac: "996511",
              amount: 18500,
              gstRate: 5,
      ...
      await markCompliant(container, ctx, complex.invoice.id, {
        withEwb: true,
        distanceKm: 840,
        expiresInHours: 9,
        vehicleNo: "MH12QR4455",
      ...
      await invoiceService.recordPayment(ctx, paid.invoice.id, {
        amount: paid.invoice.grandTotal / 100,
        method: "rtgs",
        reference: "RTGS/2026/88120",
      });

      // 3. Inter-state, part paid, overdue.
      const partPaid = await invoiceService.createDraft(
        ctx,
        base({
          ...
          transport: {
            ...
            transportMode: 1,
            distanceKm: 620,
            vehicleNo: "MH14AB7788",
            vehicleType: "R",
            transportDocNo: "DRL/2026/43880",
            transportDocDate: daysAgo(45),
            subSupplyType: "1",
          },
        }),
      );
      await invoiceService.finalize(ctx, partPaid.invoice.id);
      await markCompliant(container, ctx, partPaid.invoice.id, {
        withEwb: true,
        distanceKm: 620,
        expiresInHours: -72,
        vehicleNo: "MH14AB7788",
      ...
      await invoiceService.recordPayment(ctx, partPaid.invoice.id, {
        amount: 250000,
        method: "neft",
        reference: "NEFT/2026/5510",
      });

      // 4. Small unregistered walk-in — no e-Invoice, no e-Way Bill.
      const walkIn = await invoiceService.createDraft(
        ctx,
        base({
      ...
      await invoiceService.recordPayment(ctx, walkIn.invoice.id, {
        amount: walkIn.invoice.grandTotal / 100,
        method: "cash",
      });

      // 5. A services invoice — no movement of goods.
      const services = await invoiceService.createDraft(
        ctx,
        base({
      ...
      const cancelled = await invoiceService.createDraft(
        ctx,
        base({
      ...
      logger.info(
        { invoices: summary.total, customers: customers.length, products: products.size },
        "sample data created",
      );
      logger.info(
...
async function markCompliant(
  ...
  const [invoice] = await database.db
    .select()
    .from(invoices)
  ...
  await database.db
    ...
    .set({
      ...
      status: invoice.status === "pending" ? "generated" : invoice.status,
    })
    .where(eq(invoices.id, invoiceId));
