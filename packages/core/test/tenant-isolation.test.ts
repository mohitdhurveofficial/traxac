...
describe("tenant isolation", () => {
  ...
  it("hides another business's invoice from get()", async () => {
    await expect(container.invoices.get(beta.ctx, alphaInvoiceId)).rejects.toThrow(/not found/i);
  });
  ...
  it("refuses to finalize another business's invoice", async () => {
    await expect(container.invoices.finalize(beta.ctx, alphaInvoiceId)).rejects.toThrow(
      /not found/i,
    );
  });
  ...
  it("refuses to edit another business's invoice", async () => {
    await expect(
      container.invoices.updateDraft(beta.ctx, alphaInvoiceId, invoiceInput(beta)),
    ).rejects.toThrow(/not found/i);
  });

  it("refuses to cancel another business's invoice", async () => {
    await expect(container.invoices.cancel(beta.ctx, alphaInvoiceId, "attempt")).rejects.toThrow(
      /not found/i,
    );
  });
  ...
  it("hides another business's customers and products", async () => {
    ...
    await expect(container.masters.getProduct(beta.ctx, alpha.productId)).rejects.toThrow(
      /not found/i,
    );
