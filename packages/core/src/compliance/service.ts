...
export class ComplianceService {
  ...
  async generateEinvoice(
    ...
    const [gstin] = await this.db
      .select()
      .from(gstins)
      .where(scopedById(ctx, gstins, invoice.gstinId))
      .limit(1);
  ...
  async cancelEinvoice(
    ...
    const gatewayCtx = this.gatewayContext(
      ctx,
      record.gstin,
      record.environment as "sandbox" | "production",
      credentials,
  ...
  async generateEwb(
    ...
    const hasPartB = Boolean(
      options.partB?.vehicleNo ||
      invoice.vehicleNo ||
      options.partB?.transportDocNo ||
      invoice.transportDocNo,
    );
    ...
    const transporterRow =
      (options.transporterId ?? invoice.transporterId)
        ? (
            await this.db
              .select()
              .from(transporters)
              .where(
                scopedById(
                  ctx,
                  transporters,
                  (options.transporterId ?? invoice.transporterId) as string,
                ),
              )
              .limit(1)
          )[0]
  ...
  async updateEwbTransporter(
    ...
    await this.deps.audit.record(ctx, {
      action: "ewb.transporter_updated",
      entityType: "invoice",
      entityId: invoiceId,
  ...
  async expireLapsedEwbs(): Promise<number> {
    ...
    for (const row of rows) {
      await this.db
        .update(invoices)
        .set({ ewbStatus: "expired", updatedAt: new Date() })
  ...
  async readEinvoice(ctx: AuthContext, invoiceId: string): Promise<Einvoice | null> {
    const [row] = await this.db
      .select()
      .from(einvoices)
      .where(scoped(ctx, einvoices, eq(einvoices.invoiceId, invoiceId)))
      .limit(1);
    return row ?? null;
  }
  ...
  private async upsertEwb(
    ...
    if (existing && existing.status !== "cancelled") {
      const [row] = await this.db
        ...
        .where(eq(ewayBills.id, existing.id))
        .returning();
  ...
  private async recordIrpIssuedEwb(
    ...
    const now = new Date();
    const derived = validUntil ?? computeValidity({ distanceKm: 0, generatedAt: now }).validUntil;
    const record = await this.upsertEwb(ctx, invoiceId, {
      gstin,
      environment,
      status: "generated",
      distanceKm: 0,
      requestPayload: { source: "irp" },
      transporterId: null,
    ...
    await this.db
      .update(invoices)
      .set({ ewbStatus: "generated", updatedAt: now })
      .where(eq(invoices.id, invoiceId));
  ...
  private async transporterName(
    ctx: AuthContext,
    transporterId: string | null,
  ): Promise<string | null> {
    if (!transporterId) return null;
    const [row] = await this.db
      .select({ name: transporters.name })
      .from(transporters)
