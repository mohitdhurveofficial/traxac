...
import {
  apiKeys, memberships, sessions, tenants, tenantSettings, users,
...
export class AuthService {
  ...
  private async uniqueSlug(base: string): Promise<string> {
    for (let attempt = 0; attempt < 20; attempt++) {
      ...
      const hit = await this.db.select({ id: tenants.id }).from(tenants)
        .where(eq(tenants.slug, candidate)).limit(1);
  ...
  async login(
    ...
    const [user] = await this.db.select().from(users)
      .where(eq(users.email, normalised)).limit(1);
    ...
    const chosen = tenantId
      ? memberRows.find((m) => m.id === tenantId)
      : memberRows[0];
    if (!chosen) throw new AppError("FORBIDDEN", "You do not have access to that business");

    if (needsRehash(user.passwordHash)) {
      const upgraded = await hashPassword(password);
      await this.db.update(users).set({ passwordHash: upgraded, updatedAt: new Date() })
  ...
  async resolveApiKey(rawKey: string, meta: RequestMeta = {}): Promise<AuthContext | null> {
    ...
    void this.db.update(apiKeys).set({ lastUsedAt: new Date() })
      .where(eq(apiKeys.id, row.id)).catch(() => undefined);
  ...
  async logout(token: string): Promise<void> {
    await this.db.update(sessions).set({ revokedAt: new Date() })
  ...
  async inviteUser(ctx: AuthContext, input: {
    ...
    const [already] = await this.db.select().from(memberships)
  ...
  async updateMemberRole(ctx: AuthContext, userId: string, role: Role): Promise<void> {
    ...
    await this.db.update(memberships).set({ role, updatedAt: new Date() })
      .where(and(eq(memberships.userId, userId), eq(memberships.tenantId, ctx.tenantId)));
  }
  ...
  async removeMember(ctx: AuthContext, userId: string): Promise<void> {
    ...
    await this.db.update(memberships).set({ status: "disabled", updatedAt: new Date() })
      .where(and(eq(memberships.userId, userId), eq(memberships.tenantId, ctx.tenantId)));
    await this.db.update(sessions).set({ revokedAt: new Date() })
  ...
  async createApiKey(ctx: AuthContext, input: {
    name: string; role: Role; expiresAt?: Date | undefined;
  ...
  async listApiKeys(ctx: AuthContext) {
    ...
    }).from(apiKeys).where(eq(apiKeys.tenantId, ctx.tenantId));
  }

  async revokeApiKey(ctx: AuthContext, id: string): Promise<void> {
    await this.db.update(apiKeys).set({ revokedAt: new Date() })
