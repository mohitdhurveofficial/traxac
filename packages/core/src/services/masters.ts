...
import {
  branches, gstins, parties, partyAddresses, products, transporters, vehicles,
  hsnCodes, units,
} from "@traxac/database";
import { AppError, gstinStateCode, normaliseVehicleNo, toPaise } from "@traxac/shared";
import type {
  CreateBranchInput, CreateGstinInput, CreatePartyAddressInput, CreatePartyInput,
  CreateProductInput, CreateTransporterInput, CreateVehicleInput,
...
export class MastersService {
  ...
  async listGstins(ctx: AuthContext) {
    return this.db.select().from(gstins)
      .where(scoped(ctx, gstins))
      .orderBy(desc(gstins.isPrimary), asc(gstins.tradeName));
  }

  async getGstin(ctx: AuthContext, id: string) {
    const [row] = await this.db.select().from(gstins).where(scopedById(ctx, gstins, id)).limit(1);
  ...
  async createGstin(ctx: AuthContext, input: CreateGstinInput) {
    ...
    const existing = await this.db.select({ id: gstins.id }).from(gstins)
      .where(scoped(ctx, gstins, eq(gstins.gstin, input.gstin))).limit(1);
    if (existing.length) throw new AppError("CONFLICT", "This GSTIN is already registered");

    const isFirst = (await this.db.select({ n: countExpr }).from(gstins)
      .where(scoped(ctx, gstins)))[0]?.n === 0;

    return this.db.transaction(async (tx) => {
      if (input.isPrimary || isFirst) {
        await tx.update(gstins).set({ isPrimary: false }).where(eq(gstins.tenantId, ctx.tenantId));
      }
      const [row] = await tx.insert(gstins).values({
      ...
      await this.audit.record(ctx, {
        action: "gstin.created", entityType: "gstin", entityId: row.id,
  ...
  async updateGstin(ctx: AuthContext, id: string, input: Partial<CreateGstinInput> & { isActive?: boolean }) {
    requirePermission(ctx, "settings:write");
    const before = await this.getGstin(ctx, id);
    const [row] = await this.db.update(gstins)
      .set({ ...stripUndefined(input), updatedAt: new Date() })
      .where(scopedById(ctx, gstins, id)).returning();
    if (!row) throw new AppError("NOT_FOUND", "GSTIN registration not found");
    if (input.isPrimary) {
      await this.db.update(gstins).set({ isPrimary: false })
        .where(and(eq(gstins.tenantId, ctx.tenantId), ne(gstins.id, id)));
    }
    await this.audit.record(ctx, {
      action: "gstin.updated", entityType: "gstin", entityId: id,
  ...
  async createBranch(ctx: AuthContext, input: CreateBranchInput) {
    ...
    const [row] = await this.db.insert(branches).values({
      tenantId: ctx.tenantId,
    ...
    if (input.isDefault) {
      await this.db.update(branches).set({ isDefault: false })
        .where(and(eq(branches.tenantId, ctx.tenantId), eq(branches.gstinId, input.gstinId), ne(branches.id, row.id)));
    }
    await this.audit.record(ctx, {
      action: "branch.created", entityType: "branch", entityId: row.id, summary: row.name,
    });
    return row;
  }

  async updateBranch(ctx: AuthContext, id: string, input: Partial<CreateBranchInput> & { isActive?: boolean }) {
    requirePermission(ctx, "settings:write");
    const [row] = await this.db.update(branches)
      .set({ ...stripUndefined(input), updatedAt: new Date() })
      .where(scopedById(ctx, branches, id)).returning();
  ...
  async deleteBranch(ctx: AuthContext, id: string) {
    requirePermission(ctx, "settings:write");
    await this.db.update(branches).set({ isActive: false, updatedAt: new Date() })
  ...
  async listParties(ctx: AuthContext, options: ListOptions & { partyType?: string } = {}) {
    ...
    const where = scoped(ctx, parties,
      ...
      options.partyType && options.partyType !== "both"
        ...
        : undefined,
      searchAcross([parties.name, parties.legalName, parties.gstin, parties.city, parties.phone, parties.email], options.q),
    );
    const [rows, [count]] = await Promise.all([
      this.db.select().from(parties).where(where)
        .orderBy(asc(parties.name)).limit(limit).offset((page - 1) * limit),
  ...
  async getParty(ctx: AuthContext, id: string) {
    const [row] = await this.db.select().from(parties).where(scopedById(ctx, parties, id)).limit(1);
  ...
  async getPartyWithAddresses(ctx: AuthContext, id: string) {
    ...
    const addresses = await this.db.select().from(partyAddresses)
      .where(scoped(ctx, partyAddresses, eq(partyAddresses.partyId, id), eq(partyAddresses.isActive, true)))
  ...
  async createParty(ctx: AuthContext, input: CreatePartyInput) {
    ...
    const [row] = await this.db.insert(parties).values({
      tenantId: ctx.tenantId,
      name: input.name,
      ...
      defaultPlaceOfSupply: blankToNull(input.defaultPlaceOfSupply) ?? blankToNull(input.stateCode),
    ...
    await this.audit.record(ctx, {
      action: "party.created", entityType: "party", entityId: row.id, summary: row.name,
    });
    return row;
  }
  ...
  async updateParty(ctx: AuthContext, id: string, input: Record<string, unknown>) {
    ...
    const [row] = await this.db.update(parties)
      .set({ ...stripUndefined(input), updatedAt: new Date() })
      .where(scopedById(ctx, parties, id)).returning();
    if (!row) throw new AppError("NOT_FOUND", "Customer not found");
    await this.audit.record(ctx, {
      action: "party.updated", entityType: "party", entityId: id, diff: diffRecords(before, row),
    });
    return row;
  }
  ...
  async archiveParty(ctx: AuthContext, id: string) {
    requirePermission(ctx, "parties:write");
    await this.db.update(parties).set({ isActive: false, updatedAt: new Date() })
  ...
  async addPartyAddress(ctx: AuthContext, partyId: string, input: CreatePartyAddressInput) {
    ...
    const [row] = await this.db.insert(partyAddresses).values({
      tenantId: ctx.tenantId,
      partyId,
      label: input.label,
      kind: input.kind,
    ...
    if (input.isDefault) {
      await this.db.update(partyAddresses).set({ isDefault: false })
        .where(and(
  ...
  async deletePartyAddress(ctx: AuthContext, id: string) {
    requirePermission(ctx, "parties:write");
    await this.db.update(partyAddresses).set({ isActive: false })
  ...
  async listProducts(ctx: AuthContext, options: ListOptions = {}) {
    const limit = options.limit ?? 25;
    const page = options.page ?? 1;
    const where = scoped(ctx, products,
    ...
    const [rows, [count]] = await Promise.all([
      this.db.select().from(products).where(where)
        .orderBy(asc(products.name)).limit(limit).offset((page - 1) * limit),
  ...
  async getProduct(ctx: AuthContext, id: string) {
    const [row] = await this.db.select().from(products).where(scopedById(ctx, products, id)).limit(1);
  ...
  async createProduct(ctx: AuthContext, input: CreateProductInput) {
    ...
    const clash = await this.db.select({ id: products.id }).from(products)
      .where(scoped(ctx, products, eq(products.name, input.name))).limit(1);
    ...
    await this.audit.record(ctx, {
      action: "product.created", entityType: "product", entityId: row.id, summary: row.name,
    });
    return row;
  }
  ...
  async updateProduct(ctx: AuthContext, id: string, input: Record<string, unknown>) {
    ...
    const [row] = await this.db.update(products)
      ...
      .where(scopedById(ctx, products, id)).returning();
    if (!row) throw new AppError("NOT_FOUND", "Product not found");
    await this.audit.record(ctx, {
      action: "product.updated", entityType: "product", entityId: id, diff: diffRecords(before, row),
    });
    return row;
  }
  ...
  async archiveProduct(ctx: AuthContext, id: string) {
    ...
    await this.audit.record(ctx, { action: "product.archived", entityType: "product", entityId: id });
  }
  ...
  async listTransporters(ctx: AuthContext, options: ListOptions = {}) {
    ...
    const page = options.page ?? 1;
    const where = scoped(ctx, transporters,
    ...
    const [rows, [count]] = await Promise.all([
      this.db.select().from(transporters).where(where)
        .orderBy(asc(transporters.name)).limit(limit).offset((page - 1) * limit),
  ...
  async getTransporter(ctx: AuthContext, id: string) {
    const [row] = await this.db.select().from(transporters)
      .where(scopedById(ctx, transporters, id)).limit(1);
  ...
  async createTransporter(ctx: AuthContext, input: CreateTransporterInput) {
    ...
    const [row] = await this.db.insert(transporters).values({
      tenantId: ctx.tenantId,
      name: input.name,
    ...
    await this.audit.record(ctx, {
      action: "transporter.created", entityType: "transporter", entityId: row.id, summary: row.name,
    });
    return row;
  }
  ...
  async updateTransporter(ctx: AuthContext, id: string, input: Record<string, unknown>) {
    requirePermission(ctx, "logistics:write");
    const [row] = await this.db.update(transporters)
      .set({ ...stripUndefined(input), updatedAt: new Date() })
      .where(scopedById(ctx, transporters, id)).returning();
    if (!row) throw new AppError("NOT_FOUND", "Transporter not found");
    return row;
  }
  ...
  async archiveTransporter(ctx: AuthContext, id: string) {
    requirePermission(ctx, "logistics:write");
    await this.db.update(transporters).set({ isActive: false, updatedAt: new Date() })
  ...
  async listVehicles(ctx: AuthContext, options: ListOptions = {}) {
    const limit = options.limit ?? 50;
    const page = options.page ?? 1;
    const where = scoped(ctx, vehicles,
    ...
    const [rows, [count]] = await Promise.all([
      this.db.select().from(vehicles).where(where)
        .orderBy(desc(vehicles.createdAt)).limit(limit).offset((page - 1) * limit),
  ...
  async createVehicle(ctx: AuthContext, input: CreateVehicleInput) {
    ...
    const [existing] = await this.db.select().from(vehicles)
      .where(scoped(ctx, vehicles, eq(vehicles.vehicleNo, vehicleNo))).limit(1);
    if (existing) return existing;
    const [row] = await this.db.insert(vehicles).values({
      tenantId: ctx.tenantId,
      vehicleNo,
  ...
  async updateVehicle(ctx: AuthContext, id: string, input: Record<string, unknown>) {
    ...
    const [row] = await this.db.update(vehicles).set(values)
      .where(scopedById(ctx, vehicles, id)).returning();
  ...
  async archiveVehicle(ctx: AuthContext, id: string) {
    requirePermission(ctx, "logistics:write");
    await this.db.update(vehicles).set({ isActive: false })
