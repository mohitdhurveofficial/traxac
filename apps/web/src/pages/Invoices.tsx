  ...
  const alerts = [
    ...
    attention?.ewbExpiringSoon
      ? {
          ...
          to: "/invoices?ewbStatus=generated",
          tone: "warn" as const,
        }
      : null,
    attention?.ewbPending
      ? {
          ...
          to: "/invoices?ewbStatus=pending",
          tone: "warn" as const,
        }
      : null,
    attention?.overdue
      ? {
          label: `${attention.overdue} overdue payment${attention.overdue > 1 ? "s" : ""}`,
          to: "/reports",
          tone: "warn" as const,
        }
      : null,
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="-mx-1 flex gap-1 overflow-x-auto px-1">
            {FILTERS.map((filter) => (
              ...
              >
                {filter.label}
                {filter.key === "draft" && attention?.drafts ? ` (${attention.drafts})` : ""}
              </button>
            ))}
          </div>
        </div>
                    ...
                    >
                      ...
                      <td className="px-4 py-3">
                        <div className="flex flex-wrap items-center gap-1.5">
                          ...
                          <EwbStatus
                            status={invoice.ewbStatus}
                            validUntil={invoice.ewbValidUntil}
                          />
                        </div>
                      </td>
                    </tr>
              ...
              {invoices.data?.items.map((invoice) => (
                <li key={invoice.id}>
                  <Link
                    ...
                    className="card block p-3.5 active:bg-slate-50"
                  >
              ...
              <div className="mt-4 flex items-center justify-between text-sm">
                ...
                <div className="flex gap-2">
                  ...
                  <button
                    type="button"
                    className="btn-secondary"
                    disabled={!invoices.data?.hasMore}
                    onClick={() => setPage((p) => p + 1)}
                  >
                    Next
                  </button>
