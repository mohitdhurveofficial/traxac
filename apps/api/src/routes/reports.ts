...
import { z } from "zod";
import { financialYear, financialYearEnd, financialYearStart, toIsoDate, toRupees } from "@traxac/shared";
...
export async function reportRoutes(app: FastifyInstance): Promise<void> {
  ...
  app.get("/export/sales-register.csv", async (request, reply) => {
    ...
    const csv = toCsv(
      ["Invoice No", "Date", "Type", "Buyer", "Buyer GSTIN", "POS", "Taxable",
        "CGST", "SGST", "IGST", "Cess", "Total", "Status", "IRN", "Ack No", "EWB No"],
      rows.map((r) => [
        r.invoiceNumber, toIsoDate(r.invoiceDate), r.docType, r.buyerName, r.buyerGstin ?? "",
        r.placeOfSupply, money(r.taxableValue), money(r.cgst), money(r.sgst), money(r.igst),
        money(r.cess), money(r.grandTotal), r.status, r.irn ?? "", r.ackNumber ?? "", r.ewbNumber ?? "",
      ]),
    );
    return sendCsv(reply, `sales-register-${toIsoDate(window.from)}-to-${toIsoDate(window.to)}.csv`, csv);
  });
  ...
  app.get("/export/hsn-summary.csv", async (request, reply) => {
    ...
    const csv = toCsv(
      ...
      rows.map((r) => [
        r.hsnSac, r.unit, r.gstRate, r.quantity, money(r.taxableValue), money(r.cgst),
        money(r.sgst), money(r.igst), money(r.cess), money(r.total),
      ]),
    );
    return sendCsv(reply, `hsn-summary-${toIsoDate(window.from)}-to-${toIsoDate(window.to)}.csv`, csv);
  });
  ...
  app.get("/export/ewb-register.csv", async (request, reply) => {
    ...
    const csv = toCsv(
      ["EWB No", "Status", "Generated", "Valid Until", "Distance km", "Vehicle",
        "Transporter", "Invoice No", "Invoice Date", "Buyer", "Value"],
      rows.map((r) => [
        r.ewbNumber ?? "", r.status,
        r.generatedAt ? toIsoDate(r.generatedAt) : "",
        r.validUntil ? toIsoDate(r.validUntil) : "",
        r.distanceKm ?? "", r.vehicleNo ?? "", r.transporterName ?? "",
        r.invoiceNumber, toIsoDate(r.invoiceDate), r.buyerName, money(r.grandTotal),
      ]),
    );
    return sendCsv(reply, `eway-bills-${toIsoDate(window.from)}-to-${toIsoDate(window.to)}.csv`, csv);
  });
}
